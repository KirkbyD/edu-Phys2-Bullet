	...
	for (std::vector<nPhysics::iPhysicsComponent*>::iterator i = PhysicsComponents.begin(); i != PhysicsComponents.end(); i++)
	{
		if (*i == component) {
			...
			if (component->GetParentID() != NULL) {
				// TODO - mediator command
				((cComplexObject*)component->GetParentPointer())->RemovePhysicsComponent(component);
			}
			// Remove from world
...
nPhysics::iBallComponent* cPhysicsManager::FactoryCreateBall(const nPhysics::sBallDef& def) {
	auto compPtr = PhysicsFactory->CreateBall(def, (Module_Hex_Value | System_Hex_Value | next_UniqueComponentID++));
	PhysicsComponents.push_back(compPtr);
	return compPtr;
}
...
nPhysics::iPlaneComponent* cPhysicsManager::FactoryCreatePlane(const nPhysics::sPlaneDef& def) {
	auto compPtr = PhysicsFactory->CreatePlane(def, (Module_Hex_Value | System_Hex_Value | next_UniqueComponentID++));
	PhysicsComponents.push_back(compPtr);
	return compPtr;
}
